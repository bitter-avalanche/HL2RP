...
function PLUGIN:PlayerUse(player, entity)
	...
	if (player:KeyDown(IN_SPEED) and Clockwork.entity:IsDoor(entity)) then
		if ((Schema:PlayerIsCombine(player) or player:GetFaction() == FACTION_ADMIN or player:HasItemByID("cwu_card") or player:HasItemByID("union_card")) and IsValid(entity.unionLock)) then
