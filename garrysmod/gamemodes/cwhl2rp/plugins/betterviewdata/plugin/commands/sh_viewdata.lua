	...
	if (Schema:PlayerIsCombine(player)) then
		...
		if (target) then
            ...
            if (data == nil or (type(data) == "string" and string.len(data) == 0)) then
      			target:SetCharacterData("combinedata", PLUGIN.config.defaultCharData);
      		end;

				Clockwork.datastream:Start( player, "EditData", {
					target,
					target:GetCharacterData("combinedata") or "",
					target:GetCharacterData("combinepoints") or {},
					target:GetCharacterData("citizenid") or "",
					target:GetCharacterData("combinepointsdel") or {}
				});
