...
function ITEM:OnChangedClothes(player, bIsWearing)
	if (bIsWearing) then
		...
		-- Change their identity status to concealed, if the suit would make them so
		...
		-- Change identity state back to visible
		player:SetSharedVar("IsConcealed", false);
	end;

	if (self.OnChangedArmorClothes) then
		self:OnChangedArmorClothes(player, bIsWearing);
	end;
end;
...
function ITEM:OnPlayerUnequipped(player, extraData)
	if (player.filterItem != nil) then
		player.filterItem:OnPlayerUnequipped(player);
	end;
