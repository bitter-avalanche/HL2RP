...
if (CLIENT) then
	...
	function ITEM:GetEntityMenuOptions(entity, options)
		...
		options["Customize"] = function()
			...
			Derma_StringRequest("Model", "Model of the item? Current: "..self("replacement", "models/error.mdl"), self("replacement", ""), function(text)
				if (!text or text == "") then
					text = self("replacement", "models/error.mdl");
				end;
				customData.Model = text;
				
			Derma_StringRequest("Weight", "Weight of the item? Current: "..self("actualWeight", "0"), self("actualWeight", "0"), function(text)
				text = tonumber(text);
				if (!text) then
			...
			Derma_StringRequest("Inventory Space", "Amount of weight the clothes can carry in their pockets? Current: "..self("invSpace", "0"), self("invSpace", "0"), function(text)
				text = tonumber(text);
				if (!text) then
					text = self("invSpace", 0);
				end;
				customData.InvSpace = math.Clamp(text, 0, 16);
			
			Derma_StringRequest("Max Armor", "Max Armor of the item? Current: "..self("maxArmor", "0"), self("maxArmor", "0"), function(text)
				text = tonumber(text);
				if (!text) then
					text = self("maxArmor", 0);
				end;
				customData.MaxArmor = math.Clamp(text, 0, 150);
			...
			Derma_StringRequest("Protection", "Protection of the item? Current: "..self("protection", "0"), self("protection", "0"), function(text)
				text = tonumber(text);
				if (!text) then
					text = self("protection", 0);
				end;
				customData.Prot = math.Clamp(text, 0, 0.5);

			Derma_StringRequest("Repair Item", "What item is needed to repair this? Current: "..self("repairItem", ""), self("repairItem", ""), function(text)
				if (!text) then
					text = self("repairItem", nil);
				end;
				customData.RepairItem = text;

			Derma_StringRequest("Repair Amount", "How much does each repair item repair? Current: "..self("repairAmount", 0), self("repairAmount", 0), function(text)
				text = tonumber(text);
				if (!text) then
					text = self("repairAmount", nil);
				end;
	...
	function ITEM:OnInstantiated()
		...
		self:SetData("MaxArmor", math.Clamp(self:GetData("MaxArmor", 0), 0, 150));
		self:SetData("InvSpace", math.Clamp(self:GetData("InvSpace", 0), 0, 16));
		self:SetData("Prot", math.Clamp(self:GetData("Prot", 0), 0, 0.5));
	end;
end;
